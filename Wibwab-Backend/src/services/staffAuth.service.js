// services/staffAuth.service.js — ลืมรหัสผ่านฝั่ง Staff ด้วยรหัส OTP 6 หลัก ส่งอีเมลจริง (ไม่จำลอง)
// โครงเดียวกับ adminAuth.service.js ต่างกันแค่กรอง role='staff' และข้อความ
...
// ขอรหัส OTP — ต้องเป็นอีเมลที่ตรงกับบัญชี staff ที่มีอยู่จริงเท่านั้น (แจ้ง error ตรง ๆ ตามที่ต้องการ
// ต่างจากฝั่งลูกค้าที่ตอบเหมือนกันทุกกรณีเพื่อกันเดาอีเมล เพราะนี่เป็นเครื่องมือภายในของพนักงาน ไม่ใช่หน้าสาธารณะ)
async function requestPasswordResetOtp(email) {
  if (!isEmail(email)) throw httpError(400, 'รูปแบบอีเมลไม่ถูกต้อง');

  const [rows] = await pool.execute("SELECT id, full_name FROM users WHERE email = ? AND role = 'staff'", [email]);
  if (rows.length === 0) throw httpError(404, 'ไม่พบอีเมลนี้ในระบบพนักงาน');
  ...
  await sendMail({
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto;">
        <h2 style="color:#0f766e;">วิบวับ — รีเซ็ตรหัสผ่านพนักงาน</h2>
        <p>สวัสดีคุณ ${rows[0].full_name},</p>
        <p>รหัส OTP สำหรับรีเซ็ตรหัสผ่านของคุณคือ:</p>
        <p style="font-size:32px; font-weight:bold; letter-spacing:6px; color:#0f766e;">${otpCode}</p>
...
async function resetPasswordWithOtp({ email, otp_code, new_password }) {
  ...
  const [users] = await pool.execute("SELECT id FROM users WHERE email = ? AND role = 'staff'", [email]);
  if (users.length === 0) throw httpError(404, 'ไม่พบอีเมลนี้ในระบบพนักงาน');
